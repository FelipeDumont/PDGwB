...
int AStar::FindRouteAStar(Dungeon* dun, int matrixOffset) {
    ...
    // The starting location is room (0,0), but offseted
    ...
    while (openList.size() > 0) {
        ...
        if (std::find_if(closedList.begin(), closedList.end(), [&](Location* l) {
        ...
        for (Location* adjacentSquare : adjacentSquares) {
            ...
            if (std::find_if(closedList.begin(), closedList.end(), [&](Location* l) {
                ...
                delete adjacentSquare; // Not used, then removed
                continue;
            }
            
            // If it's not in the open list...
            if (std::find_if(openList.begin(), openList.end(), [&](Location* l) {
                return l->X == adjacentSquare->X && l->Y == adjacentSquare->Y;
            }) == openList.end()) {
