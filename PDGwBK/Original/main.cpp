...
extern "C"
void CreateDungeons(double *rarray, double *aarray) {
    GA ga;
    // Inner Configuration
    Constants::useDFS = true;
    Constants::ID = 0;
    ...
    Constants::randGen = std::mt19937(Constants::seed);
    Constants::analisis = static_cast<int>(rarray[8]) == 1 ? true: false;
    Constants::useDFS = static_cast<int>(rarray[9]) == 1 ? false: true;
    // std::cout << (Constants::useDFS ? "TRUE" : "FALSE") << std::endl;

    ga.testingMode = false;
    Dungeon* result = ga.CreateDungeon();
